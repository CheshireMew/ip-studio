...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet_kit.py prepare <character-kit> --adapter codex-v2
```

`prepare` 会验证角色包，使用角色名、当前版本和完整角色档案建立维护侧快照，再用主参考图与少量必要身份锚点生成桌宠运行目录、布局辅助图、十三个视觉任务及各自提示词。完整档案不复制进每个生图提示词。默认目录为：

```text
<character-kit>/derivatives/pets/<pet-id>/adapters/codex-v2/runs/rNNN/
```
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet_kit.py ready <run-dir>
```
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet_kit.py accept-job <run-dir> \
  --job <job-id> \
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/derive_running_left_from_running_right.py \
  --run-dir <run-dir> \
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/extract_strip_frames.py \
  --decoded-dir <run-dir>/decoded \
  --output-dir <run-dir>/frames \
  --states all \
  --method auto

"<workspace-python>" -B <pet-studio-skill>/scripts/pet/inspect_frames.py \
  --frames-root <run-dir>/frames \
  --json-out <run-dir>/qa/review.json \
  --require-components

"<workspace-python>" -B <pet-studio-skill>/scripts/pet/compose_atlas.py \
  --frames-root <run-dir>/frames \
  --output <run-dir>/final/spritesheet.png \
  --webp-output <run-dir>/final/spritesheet.webp

"<workspace-python>" -B <pet-studio-skill>/scripts/pet/make_contact_sheet.py \
  <run-dir>/final/spritesheet.webp \
  --output <run-dir>/qa/contact-sheet.png

"<workspace-python>" -B <pet-studio-skill>/scripts/pet/render_animation_previews.py \
  --frames-root <run-dir>/frames \
...
随后把这四种姿态作为方向含义真源，整行生成第 9 行；相邻格按 22.5 度均匀插值。第 9 行接受后，确定性脚本会把八个姿态用同一尺度、下身锚点和基线注册到正式格子。看过注册结果并确认没有反转、跳位、身份变化、配件断裂或错误象限后，再生成第 10 行。第 10 行同时读取四向锚点和已完成第 9 行，使 `157.5 → 180` 与 `337.5 → 000` 也只移动一步；它必须服从第 9 行已经批准的尺度和注册，无法装入时重做第 10 行，不能回缩或改写第 9 行。
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/despill_chroma_edges.py \
...
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/validate_atlas.py \
...
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/make_direction_qa_sheet.py \
  <run-dir>/final/spritesheet-extended.webp \
  --output <run-dir>/qa/look-directions.png

"<workspace-python>" -B <pet-studio-skill>/scripts/pet/make_direction_blind_qa_sheet.py \
  <run-dir>/final/spritesheet-extended.webp \
  --output <run-dir>/qa/direction-blind-pairs.png \
  --answer-key <run-dir>/qa/direction-blind-answer-key.json

"<workspace-python>" -B <pet-studio-skill>/scripts/pet/measure_direction_continuity.py \
  <run-dir>/final/spritesheet-extended.webp \
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/combine_direction_blind_verdicts.py \
...
"<workspace-python>" -B <pet-studio-skill>/scripts/pet/validate_direction_blind_verdicts.py \
  --answer-key <run-dir>/qa/direction-blind-answer-key.json \
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet_kit.py finalize <run-dir>
"<workspace-python>" -B <pet-studio-skill>/scripts/pet_kit.py check <run-dir> --stage final
```
...
```text
"<workspace-python>" -B <pet-studio-skill>/scripts/pet_kit.py install <run-dir>
```

若相同内容已经安装，命令直接报告已存在。若同一 `pet-id` 已被其它内容占用，只把“是否替换现有宠物”交给用户；获批后增加 `--replace-installed`，旧目录会先移动到 Codex 宠物目录内的 `.pet-studio-backups/`，再安装新版本。
