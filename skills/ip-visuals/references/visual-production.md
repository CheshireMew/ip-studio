...
“极简手绘 IP”是封面、说明图和正文插图可选的内容视觉语言，不是头像、主页视觉或所有角色的全局默认值。只有用户明确选择时才采用。已有角色仍保留主轮廓、脸部锚点、主配色落点和标志物；若极简化会改变这些固定身份特征，先返回角色修改流程建立新版本，不能在衍生图里偷偷改画法。
...
衍生视觉默认只生成一版。生图前先展示实际完整提示词和图片输入，等待用户确认；确认后不得改写。图像入口正常返回可读取的图片后，直接归档并交给用户。Agent 不先做审美筛选，不因为自己的判断修改提示词、自动重生、生成候选或替用户挑图，也不把某次问题升级成以后每张图都要追加的检查表。

只有文件损坏、格式错误或图像入口明确失败时，当前任务才不算正常生成；重试仍要重新展示提示词和图片输入并等待确认。用户明确要求检查、修正、比较或授权 Agent 代选时，先按用户指出的问题看图，再按第 9 节建立一次有边界的修订。新的提示词和图片输入仍须先展示并重新确认。需要改变角色固定特征时，返回 `$ip-studio` 升版，不在衍生视觉里偷偷改身份。
