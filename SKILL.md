...
用已选择的图像入口实际执行命令输出的 `prompt` 和有序 `image_references`，不另写平行的角色或内容提示词。实际打开结果，按 reference 检查角色一致性、内容事实、动作因果、唯一结构、移动端层级、文字与 logo、技术完整性，以及所选视觉语言的专项减法规则。失败时修正最早出错的简报判断，从原始档案、内容和参考图重生；每张结果最多自动重试两次。需要改变事实、叙事重点或固定特征时只询问这一项。
