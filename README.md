...
它负责补全角色与运行合同、生图、版本管理和文件校验。角色身份与动态结果会进行所需的看图检查；普通静态衍生视觉在生成后直接交付，不进入角色库，也不会自动筛选或重生。你只需要决定会改变角色身份、内容重点或既有安装的事项。

## 你会得到什么

- **建立或修改角色**：得到完整角色档案、角色说明、单张正式主参考图和可回读的历史版本。
- **制作衍生视觉**：得到头像、主页横幅、资料卡、文章封面、说明图或正文插图，以及生成前可确认的完整提示词和有序图片输入；成图直接交付。
...
静态衍生视觉、动态角色素材和桌宠都可以读取角色包作为共同身份输入，但只有角色身份和真正可运行的动态包需要长期保存。只用一张已认可角色图制作头像、横幅、封面、说明图或正文插图时也可以直接生成，不强迫先建角色包；无论采用哪种角色来源，普通静态成图都不会写回角色库。
...
这类结果直接使用上传角色图，不建立角色包。需要复用既有角色身份时可以提供角色包路径：

```text
$ip-studio 使用 E:\path\to\character-kit，为这篇文章制作一张 5:2 封面。文章内容如下：……
```

角色包只提供当前主参考图和角色版本，生成的静态图片不会归档到角色包。没有指定保存位置时直接交付图像入口输出；指定位置时只复制最终图片到那里。所有衍生视觉都接受用户指定的任意合法 `宽:高` 比例，示例比例只在没有指定时作为默认值。

IP Studio 会把角色参考图、本次成品需要的完整内容材料、必要输出形式和你的硬要求整理成一份实际提示词，在生图前完整展示并等待确认，确认后原样交给图像模型。默认文章封面直接读取 `references/cover-prompt.md`；OKX 与 Binance 品牌编辑视觉则把提炼后的简短风格文字和完整原文交给图像模型，让它在生成前自行判断最值得表达的内容、必须保留和可以省略的信息以及适合的版式，不由 Agent 提前截取标题和数字。项目、仓库、网页或 README 只是取材来源，不会自动整份进入提示词。生成结果直接展示并交付，不再增加“认可候选后归档”的第二轮。仓库当前内置“极简手绘 IP”“OKX Editorial”和“Binance Editorial”三套可选内容视觉语言。例如：

```text
$ip-studio 使用 E:\path\to\character-kit，根据下面的定稿做一张 1:1 OKX 风格社交配图，把我的 IP 角色融入画面：……
```

这类请求会验证内置 `style-profile.json`，实际生图提示词只使用其中已经提炼好的 `generation_text`，不复制完整 JSON，也不预选构图或场景。七张来源案例不会在日常生成时打开或传给生图模型；图片输入默认包含角色主图、精确 OKX 白色标记和当前任务素材。

币安风格使用同一套风格资料维护方式，实际生图提示词使用提炼后的币安品牌编辑视觉文字，不预选黄底或深色模式，也不替图像模型决定场景家族。五张来源图中的活动文案、奖励、人物、二维码、界面和物件不会进入新作品；图片输入默认提供黄色和黑色两份精确 Binance 标记，由图像模型根据最终底色二选一使用。
...
制作一般动态角色或桌宠时，工作区 Python 环境需要提供 Pillow 和 NumPy。角色档案和普通静态视觉提示脚本只使用 Python 标准库。
...
2. 静态衍生视觉只读取当前主参考图，不把成图或输入快照写回角色库；动态角色和桌宠按运行时合同保存所需快照。
3. 当次动作、构图、场景和氛围不会反向写回角色身份。
4. 角色版本和动态运行时包保留合同要求的输入、校验值和历史；普通静态成图不建立版本链。
...
`SKILL.md` 是工作流、权限边界和交付标准的正式入口；四个顶层脚本分别负责角色包、静态视觉提示与文章配图计划、通用动态素材和 Codex 桌宠适配器的机器可验证合同。修改工作流后，先检查四套命令入口仍可读取：
...
角色的最终验收必须读取脚本正式输出并实际看图；静态衍生视觉要核对正式提示词、有序图片输入和图像入口真实输出，并确认没有写入角色库；一般动态素材还要验证整表生产者、透明帧、图集、manifest 和目标消费者；桌宠继续验证完整 8×11 精灵表、九组动画预览、十六方向语义、盲审和安装目录。只运行帮助命令或静态检查不能代替这些真实链路。

## 隐私

Skill 默认只读取当前请求、当前对话、用户明确提供的材料、角色文件夹，以及动态任务中与状态生产、图片消费和运行时接入直接相关的项目文件。角色包、角色 QA 文件和动态运行记录保存在本仓库的 `ip-studio-output/` 本地工作区。普通静态图片保留在图像入口的输出位置，或只复制到用户明确指定的位置，不会放进角色库。`ip-studio-output/` 已被 Git 忽略，不属于源码，也不会被默认提交。
