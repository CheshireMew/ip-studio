...
IP Studio 会把角色参考图、完整内容、必要输出形式和你的硬要求整理成一份实际提示词，在生图前完整展示并等待确认，确认后原样交给图像模型。默认文章封面直接读取 `references/article-cover-prompt.md`：Agent 只交付完整文章、图片职责和比例，由图像模型在同一次生成中判断核心对象、关键变化、标题、角色动作与视觉叙事。第一张正常结果直接交付或归档；只有你明确要求检查或修订时才进入下一版。仓库当前内置“极简手绘 IP”“OKX Editorial”和“Binance Editorial”三套可选内容视觉语言。例如：
