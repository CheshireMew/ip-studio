...
在 `SKILL.md` 已确认用户要把已锁定角色制作成 Codex 桌宠后读取本文件。它负责桌宠适配、生图任务、精灵表拼装、动态与方向检查、成品目录和本机安装；角色身份仍由角色包中的 `character-profile.json` 与单张主参考图决定。
...
先调用工作区依赖加载器，并把返回的 Python 绝对路径用于本文件的所有命令。进入生图前加载当前环境的 `$imagegen` Skill；基础图、动作条和方向条都只通过它生成，确定性脚本只负责布局、抠图、拼装、检查和归档。
...
`base` 是桌宠尺寸下的视觉基准，不替代角色主参考图。它必须是单角色、完整全身、居中、清楚轮廓、纯色幕布，无文字、场景、阴影、辉光和无连接装饰。接受 `base` 后，命令会建立 `references/canonical-base.png`；之后每个动作条都同时读取角色主参考图和这张桌宠基准。
...
实际打开标准总览和九个 GIF。身份漂移、空帧、身体裁切、帧间缩放跳动、方向错误、步态倒放、几乎静止的循环或状态含义混淆都需要重做对应整行。若源动作条本身尺度稳定、跳动只来自逐帧裁切，可用 `--method stable-slots` 重新抽帧并在 `inspect_frames.py` 中增加 `--allow-stable-slots`；这是一项看过 GIF 后的修正，不是默认。
...
`finalize` 在写入成品前重新验证角色快照、十三个视觉任务、九个 GIF、8×11 精灵表、去色、标准动作、方向语义、三份盲审和独立最终看图。通过后生成：
