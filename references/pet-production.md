...
随后把这四种姿态作为方向含义真源，整行生成第 9 行；相邻格按 22.5 度均匀插值。第 9 行接受后，确定性脚本会把八个姿态用同一尺度、下身锚点和基线注册到正式格子。看过注册结果并确认没有反转、跳位、身份变化、配件断裂或错误象限后，再生成第 10 行。第 10 行同时读取四向锚点和已完成第 9 行，使 `157.5 → 180` 与 `337.5 → 000` 也只移动一步。已经批准的第 9 行及其注册尺度不再改变；第 10 行必须沿用同一尺度、锚点和基线，无法安全装入正式格子时重做完整第 10 行，不能反过来缩小第 9 行。
