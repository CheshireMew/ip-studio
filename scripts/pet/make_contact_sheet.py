...
def main() -> None:
    ...
    block_height = LABEL_HEIGHT + len(backgrounds) * cell_h
    ...
    for row in range(rows):
        ...
        for background_index, (background_name, background_color) in enumerate(backgrounds):
            preview_y = y + LABEL_HEIGHT + background_index * cell_h
            for column in range(COLUMNS):
                ...
                if background_color is None:
                    ...
                    bg = Image.new("RGB", (cell_w, cell_h), background_color)
                bg.paste(crop, (0, 0), crop)
                x = column * cell_w
                sheet.paste(bg, (x, preview_y))
                outline = "#18a058" if is_used_cell(rows, row, column) else "#cc3344"
                draw.rectangle(
                    (x, preview_y, x + cell_w - 1, preview_y + cell_h - 1),
                    outline=outline,
                )
                label_color = "#ffffff" if background_name in {"dark", "contrast"} else "#111111"
                draw.text((x + 4, preview_y + 4), str(column), fill=label_color, font=font)
            draw.rectangle((0, preview_y, 57, preview_y + 15), fill="#111111")
            draw.text((4, preview_y + 3), background_name, fill="#ffffff", font=font)
