...
from chroma_matte import dilate, key_family_mask
...
def chroma_fringe_count(
    ...
    *,
    ...
    alpha = arr[:, :, 3]
    near_transparency = dilate(alpha == 0, edge_radius)
    key_family = key_family_mask(
        arr[:, :, :3],
        chroma_key,
        minimum_dominance=18,
        minimum_similarity=0.9,
    )
    return int(((alpha >= alpha_minimum) & near_transparency & key_family).sum())
...
def main() -> None:
    ...
    parser.add_argument("--max-chroma-fringe-pixels", type=int, default=320)
        ...
        for column_index in range(COLUMNS):
            ...
            if used and chroma_fringe_pixels > args.max_chroma_fringe_pixels:
                message = (
                    ...
                    f"visible boundary pixels in the chroma-key color family {args.chroma_key}; "
                    "inspect the dark and light contact-sheet rows for a colored halo"
                )
