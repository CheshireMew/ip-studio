...
from chroma_matte import ALGORITHM as CHROMA_MATTE_ALGORITHM
from chroma_matte import matte_chroma_background
...
def remove_chroma_background(
    ...
    matte, _metrics = matte_chroma_background(
        image,
        chroma_key=chroma_key,
        threshold=min(threshold, 72.0),
        softness=max(72.0, threshold),
    )
    return matte
...
def write_manifest(
    ...
    manifest = {
        ...
        "chromaMatte": {
            "algorithm": CHROMA_MATTE_ALGORITHM,
            "key": f"#{chroma_key[0]:02X}{chroma_key[1]:02X}{chroma_key[2]:02X}",
            "threshold": min(chroma_threshold, 72.0),
            "softness": max(72.0, chroma_threshold),
        },
        "spritesheetLayout": {
            "columns": COLUMNS,
            "rows": EXTENDED_ROWS,
...
def main() -> None:
    ...
    if args.registered_row_9:
        ...
        if target is None:
            raise SystemExit("neutral reference cell must contain visible pixels")
        registered_scale = load_registration_scale(
            Path(args.row_9_registration).expanduser().resolve()
        )
        row_10_safe_scale = normalization_scale(row_10_cells, target)
        final_scale = min(registered_scale, row_10_safe_scale)
        if final_scale < registered_scale:
            backoff = final_scale / registered_scale
            row_9_cells = normalize_cells_to_reference(
                row_9_cells,
                neutral,
                backoff,
            )
            print(
                "applied uniform 16-direction scale backoff "
                f"{registered_scale:.6f}->{final_scale:.6f}"
            )
