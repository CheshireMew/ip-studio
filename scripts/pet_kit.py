...
class PetError(ValueError):
    """Raised when a pet run or package violates the IP Studio contract."""
...
def _prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if failed.exists():
            archive_root = destination.parent / ".ip-studio-failed"
...
def _build_standard_intermediate(run: Path) -> None:
    ...
    try:
        ...
        failed_root = run / "qa" / ".ip-studio-failed"
    ...
    archived_root = run / "qa" / ".ip-studio-staging-records"
...
def main() -> int:
    ...
    except (PetError, character_kit.ProfileError) as error:
